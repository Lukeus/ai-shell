...
via the `packages/agent-runtime` package without giving the renderer or extensions
direct OS access. Today there is no clear spec for agent lifecycle, tool execution,
policy gating, or event streaming to the UI. This makes it hard to build reliable
agent runs, audit sensitive actions, and keep the renderer isolated.

## Goals
- Define the Agent Host process responsibilities and boundaries, and the
  agent-runtime implementation responsibilities.
...
- `packages/agent-runtime` contains the LangChain DeepAgents orchestration loop
  and emits contract-shaped events to the Agent Host.
...
- Agent runtime lives in `packages/agent-runtime` and hosts LangChain DeepAgents.
