...
- Agent Host process hosts the runtime process boundary and IPC wiring; it delegates
  Deep Agents orchestration to the `packages/agent-runtime` package (LangChain
  DeepAgents implementation); no direct OS access.
...
- Add Zod schemas for:
  ...
  - AgentEvent payloads (plan/todos, tool call, tool result, log, error, status).
  - ToolCallEnvelope (toolId, requesterId, input, reason, runId, todoId?, deepStepId?).
...
- `packages/agent-runtime` contains the LangChain DeepAgents implementation and
  emits contract-shaped events to the Agent Host.
- Agent Host wires IPC and forwards run lifecycle/events between main and
  agent-runtime.
...
- LangChain DeepAgents implementation lives in `packages/agent-runtime` and emits
  contract-shaped events.
